...
description: Build and sharpen a project's domain model. Use when the user wants to pin down domain terminology or a ubiquitous language (glossary), record an architectural decision, or when another skill needs to maintain the domain model.
---
...
A repo might have an optional `GLOSSARY.md` file under `docs/` directory.
...
Create files lazily — only when you have something to write. If no `docs/GLOSSARY.md` exists, create one when the first term is resolved.
